...
export async function generateBusinessCopy(payload) {
  try {
    const { data } = await client.post("/api/ai/generate-business-copy", payload);
    return data.variants || [];
  } catch (error) {
    throw new Error(parseError(error, "Unable to generate business copy."));
  }
}
